...
class FaceManager():
    ...
    def __call__(self, image):
        ...
        face_flag, face_landmarks = self.postprocess_output(net_output)

        return face_flag, face_landmarks * ratio

    def draw_keypoints(self, cv_frame, keypoints):
        for idx in range(0, 467):
            cv2.circle(cv_frame, (int(keypoints[idx, 0]), int(keypoints[idx, 1])), radius=0, color=(204, 0, 192), thickness=2)